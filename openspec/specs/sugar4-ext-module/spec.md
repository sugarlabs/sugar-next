...
Provide a GTK4-usable Python replacement for `SugarExt`/`SugarGestures`
(GTK3-compiled GObject Introspection libraries that jarabe's GTK4 port
cannot load due to an ABI conflict), so jarabe can use the small subset
of that API it still actively calls.
